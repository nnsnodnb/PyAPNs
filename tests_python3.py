...
for i in range(0, NUM_MOCK_TOKENS):
    mock_tokens.append(_ensure_bytes(hashlib.sha256(_ensure_bytes("%.12f" % random())).hexdigest()))
...
class TestAPNs(unittest.TestCase):
    ...
    def testGatewayServer(self):
        ...
        self.assertEqual(notification[0:1], b'\00')
    ...
    def testFrame(self):
        ...
        # ToDo: in Python3 dicts are not ordered, so that assertion might not pass. \
        # ToDo: Need to make test case to check unordered dict
        f = bytearray(b'\x02\x00\x00\x00t\x01\x00 \xb5\xbb\x9d\x80\x14\xa0\xf9\xb1\xd6\x1e!\xe7\x96\xd7\x8d\xcc\xdf\x13R\xf2<\xd3(\x12\xf4\x85\x0b\x87\x8a\xe4\x94L\x02\x00<{"aps":{"sound":"default","badge":4,"alert":"Hello World!"}}\x03\x00\x04\x00\x00\x00\x01\x04\x00\x04\x00\x00\x0e\x10\x05\x00\x01\n')

        self.assertEqual(str(f), str(frame))
